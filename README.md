...
- **Дата**: строка в формате DD.MM.YYYY (например, "12.12.1212")
...
├── store.ts          # Обертка для electron-store
...
- `getDoctorsList()` - получение списка врачей из конфигурации
- `getTaskBlocksForDate(date, doctor)` - получение блоков задач для даты и врача
- `getTaskBlocks()` - получение актуальной структуры блоков из meta
- `saveReport(date, doctor, blocks)` - сохранение отчета о смене
- `getConfig()` - получение полной конфигурации
- `updateConfig(config)` - обновление конфигурации
- `printReport(date, doctor)` - печать отчета о смене
...
- `ShiftReportForm.vue` - главная форма отчета о смене
- `DoctorSelector.vue` - выбор врача из списка
- `DatePicker.vue` - выбор даты с блокировкой будущих дат
- `TaskBlockList.vue` - отображение блоков задач
- `TaskBlock.vue` - отдельный блок с задачами
- `TaskItem.vue` - элемент задачи со статусом и описанием
